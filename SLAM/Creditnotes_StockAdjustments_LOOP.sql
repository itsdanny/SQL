...
DECLARE @res  TABLE(Id INT IDENTITY(1,1), SageRef varchar(10), ProductCode VARCHAR(20), Quantity VARCHAR(4), CreditAmount FLOAT, defDate VARCHAR(8), order_no VARCHAR(20), OrderLines INT)
DECLARE @orderno VARCHAR(20)
DECLARE @NextCNNumber INT


DECLARE @Orders TABLE(Id INT IDENTITY(1,1), date_entered DATETIME, customer VARCHAR(3), invoice_no VARCHAR(20), order_no VARCHAR(20))
INSERT INTO @Orders
SELECT		DISTINCT TOP 2 date_entered, customer, invoice_no, LTRIM(RTRIM(order_no))
...
BEGIN	
SELECT	@orderno = order_no FROM	@Orders WHERE Id = @OrderRow
SET @FileRow = 1
	INSERT INTO @res
	SELECT	DISTINCT	LTRIM(RTRIM(h.customer)), 
				...
				(SELECT count(DISTINCT da.product) FROM			demo.scheme.opdetm da WITH(NOLOCK) WHERE		da.order_no = @orderno) AS OrderLines
	...
	BEGIN
	INSERT INTO #CreditNotes
	SELECT '00|SRNXY|SOCREATE|Debit Notes|'
	END 
	SET @Rows = (SELECT COUNT(1) FROM @res)
	INSERT INTO #CreditNotes
			SELECT		'03|0|=CNA'+REPLACE(STR(@NextCNNumber, 5), SPACE(1), '0')+'D|3|'+SageRef+'|12|'+defDate+'|13|'+defDate+'|14|'+defDate+'|15|'+defDate+'|16|'+defDate+'|24|4|25|N|26|N|32|0|34|0|35|0|36|0|37|0|38|0|39|0|41|N|45|'+CAST(OrderLines AS varchar(4))+'|46|N|85|0|86|0|87|0|'			
			FROM		@res
			WHERE		Id = @Row
	WHILE @Row <= @Rows
	BEGIN			
				...
				BEGIN
					INSERT INTO #CreditNotes
					SELECT '04|0|=CNA'+REPLACE(STR(@NextCNNumber, 5), SPACE(1), '0')+'F|2|'  + REPLACE(STR(CAST(@FileRow AS VARCHAR(5)), 4), SPACE(1), ' ')+'|3|'+ProductCode+'|4|'+
			...
			BEGIN
				INSERT INTO #CreditNotes
					SELECT '04|0|=CNA'+REPLACE(STR(@NextCNNumber, 5), SPACE(1), '0')+'D|2|'  + REPLACE(STR(CAST(@Row AS VARCHAR(5)), 4), SPACE(1), ' ')+'|3|'+ProductCode+'|4|'+
						CASE WHEN ProductCode LIKE	'GEN%' THEN 'S|5|' else 'P|5|SL' END +'|9||20|'+CAST(Quantity*-1 AS VARCHAR(10))+'|21|'+CAST(Quantity*-1 AS VARCHAR(10))+'|22|'+CAST(Quantity*-1 AS VARCHAR(10))+'|24|' + CAST(CreditAmount AS VARCHAR(10)) + '|25|0|30|N|'
					FROM	@res
					WHERE	Id = @Row
			END
			SET @Row = @Row + 1
...
BEGIN
	INSERT INTO #StockAdjusts	
	SELECT		'24|1|SL|2|'+LTRIM(RTRIM(d.product))+'|3|'+CAST(SUM(d.order_qty*-1) AS VARCHAR(4))+'|4|'+convert(varchar, date_entered, 3) +'|6||8|DUPLICATE CREDITNOTE ('+@orderno+')|9|'+RIGHT(LTRIM(RTRIM(h.customer)),2)+'|16||21||'
	FROM		demo.scheme.opheadm h
	INNER JOIN 	demo.scheme.opdetm d
	ON			h.order_no = d.order_no
	WHERE		h.order_no = @orderno
...
--SELECT	*
--FROM		#StockAdjusts
	-- SO number reset to = 16573

UPDATE 	VapeConnectTest.dbo.SYSSettings SET KeyValue = 16574  WHERE KeyRef = 'IMPNXBRCNNUM'
